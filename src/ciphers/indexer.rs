...
impl Cipher for Indexer {
    fn generate() -> Self where Self: Sized {
        Indexer {
            ...
            multiplier: thread_rng().gen_range(-2..=2),
    ...
    fn mutate(&mut self, iteration: Option<u32>) {
        match iteration {
            ...
            Some(3) => self.multiplier += 1,
            Some(4) => self.multiplier -= 1,
            Some(5) => self.multiplier = -self.multiplier,
    ...
    fn encrypt(&self, data: &mut [u8]) {
        let ciphers: Vec<Box<dyn Cipher>> = (0..data.len())
            .map(|i| {
                let i = i as i32 * self.multiplier + self.offset;
